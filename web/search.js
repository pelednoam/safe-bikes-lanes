...
export function rank(query, candidates, opts = {}) {
    ...
    for (const c of candidates) {
        const m = Math.max(matchScore(query, c.name), c.match === undefined ? 0 : matchScore(query, c.match));
        if (m === 0)
            continue;
    ...
    for (const { row } of scored) {
        ...
        const duplicate = out.some((o) => normalise(o.name) === key &&
...
export function describe(row, fmtDist) {
    ...
    if (row.distanceM !== undefined)
        bits.push(`${fmtDist(row.distanceM)} as the crow flies`);
    return bits.join(" · ");
}
...
export const GEOCODE_MIN_GAP_MS = 1100;
/**
 ...
 */
export function worthGeocoding(query, localHits) {
    const q = query.trim();
    if (q.length < 4)
        return false;
    if (/\d/.test(q))
        return true; // a house number: only the geocoder has these
 ...
 */
export function geocodeDelayMs(now, lastAt) {
    const since = now - lastAt;
    if (since >= GEOCODE_MIN_GAP_MS)
        return 0;
