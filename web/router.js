...
export class Router {
     ...
     */
    setUpgradedPoints(points, snapM = UPGRADE_SNAP_M) {
    ...
    routeOptions(start, end, profileId = "young_kids", preferFlat = false, bias, avoid, walkMaxM = 0) {
        ...
        for (const c of candidates) {
            ...
            const payload = c.id === "direct" ? directPayload : this.payload(path, profile, walkFlags);
            ...
            ? Array.from({ length: 8 }, (_v, i) => {
                ...
                return {
                    ...
                    at: [
                        ...
                        start[1] + (Math.cos(bearing) * ideal) / 110540,
                    ],
                };
            })
            : (pois ?? [])
