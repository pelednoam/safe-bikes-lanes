 ...
 */
let onTileProgress: ((done: number, total: number) => void) | undefined;
...
async function requestRoute(): Promise<void> {
  ...
  onTileProgress = (done, total): void => {
    ...
    if (total > 4) showStage("Loading the map around your route…", `${done} of ${total}`);
  };
...
async function gradeSearchResults(
  ...
  for (const row of rows) {
    if (mine !== gradeGen) return; // a newer search owns the list now
    ...
    if (hit === undefined) {
      try {
        ...
        if (mine === gradeGen) clearGrading(row);
        continue;
      }
    }
    if (mine !== gradeGen) return;
...
function renderSearchResults(results: NominatimResult[], target: "start" | "end" = "end"): void {
  ...
  if (target === "end") void gradeSearchResults(grading);
...
map.on("click", (e: MapMouseEvent) => {
  ...
  const inspectable = ["build", "crossings"].filter(
    (id) =>
...
(function initSheet(): void {
  ...
  if (window.matchMedia("(max-width: 760px), (max-height: 500px)").matches) setSheet("peek");
  let dragging = false;
  let startY = 0;
  let startH = 0;
  let moved = 0;
...
function hideClassify(): void {
  ...
  el<HTMLDivElement>("nav-avoid-row").style.display = "none";
}
...
async function quickReport(): Promise<void> {
  ...
  el<HTMLDivElement>("nav-avoid-row").style.display = "block";
  window.clearTimeout(classifyTimer);
