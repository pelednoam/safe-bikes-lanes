...
// It lived in app.ts; the city page needed it, and copying it would have let
// the wordings drift apart silently.
import type { ProtectionClass, SafetyGrade } from "./types.js";
import { PROFILES } from "./router.js";

export const CLASS_LABELS: Record<ProtectionClass, string> = {
...
export const CLASS_SAFETY: Record<ProtectionClass, string> = {
...
export interface SegmentProps {
  // explicitly undefined-able: these come out of GeoJSON properties, where a
  // field may be absent, and the project builds with exactOptionalPropertyTypes
  cls?: ProtectionClass | undefined;
 ...
 */
export function cautionsHtml(
  cautions: { name: string; meters: number; cls: string }[],
  fmt: (m: number) => string,
): string {
  return cautions
    .map(
      (c) =>
        `<li>⚠ ${esc(c.name)}: ${esc(fmt(c.meters))} of ` +
        `${esc(CLASS_LABELS[c.cls as keyof typeof CLASS_LABELS] ?? c.cls)}</li>`,
    )
    .join("");
}
...
/** A Mapillary image, as this app asks for it. */
export interface MapillaryNear {
...
export async function nearestMapillary(
  ...
  if (photosPaused()) throw new Error("mapillary paused");
  ...
  photoFailures = 0;
  photoPausedUntil = 0;
  const data = (await resp.json()) as { data: MapillaryNear[] };
  const kx = 111_320 * Math.cos((lat * Math.PI) / 180);
  const nearest = data.data
    .map((im) => {
      const c = im.computed_geometry?.coordinates;
      const away =
        c === undefined ? Infinity : Math.hypot((c[0] - lon) * kx, (c[1] - lat) * 110_540);
      return { im, away };
    })
...
export async function fetchSegmentPhoto(
  lon: number,
  lat: number,
  token: string,
): Promise<{ url: string | null; captured: number | null }> {
  if (token === "") return { url: null, captured: null };
