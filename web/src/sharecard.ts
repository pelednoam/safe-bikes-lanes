...
function cardBase(): { canvas: HTMLCanvasElement; ctx: CanvasRenderingContext2D } | null {
...
export async function drawRideCard(ride: RideSummary): Promise<Blob> {
  const base = cardBase();
  if (base === null) throw new Error("canvas unavailable");
...
export async function drawTotalsCard(totals: RideTotals): Promise<Blob> {
  const base = cardBase();
  if (base === null) throw new Error("canvas unavailable");
  const { canvas, ctx } = base;
  ctx.fillStyle = "rgba(255,255,255,.8)";
