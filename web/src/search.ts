...
export interface Candidate {
  ...
  kind?: string;
  /** Town or street, when known: the second line that tells two Elm Streets apart. */
  context?: string;
}
...
export function matchScore(query: string, name: string): number {
  ...
  if (qs === ns) return 100;
  if (ns.startsWith(qs)) return 90; // "elm" -> "Elm Street"
  ...
  if (ordered) return 75;
  // same words, any order: "school elm" -> "Elm Street School"
  if (q.every((qw) => n.some((nw) => nw.startsWith(qw)))) return 60;
  // a word begins somewhere inside a name word: "vard" in "Harvard"
  if (q.length === 1 && ns.includes(qs)) return 40;
  return 0;
}
...
export function rank(query: string, candidates: Candidate[], opts: RankOptions = {}): Ranked[] {
  ...
  for (const { row } of scored) {
    ...
    const near = out.find(
      (o) => normalise(o.name) === key && metresBetween([o.lon, o.lat], [row.lon, row.lat]) < 250,
    );
    if (near !== undefined) continue;
    if (seen.has(`${key}|${Math.round(row.lon * 3000)}|${Math.round(row.lat * 3000)}`)) continue;
...
export function describe(row: Ranked, fmtDist: (m: number) => string): string {
  ...
  if (row.distanceM !== undefined) bits.push(`${fmtDist(row.distanceM)} away`);
  return bits.join(" · ");
}
