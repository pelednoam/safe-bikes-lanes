...
export class Router {
  ...
  private shortestPathWalkBudgeted(
    from: number,
    to: number,
    w: Float64Array,
    budgetM: number,
    ...
    for (let tries = 0; tries < 6; tries++) {
      ...
      r.walk.forEach((flag, i) => {
        if (flag) walked += this.g.edges[r.path[i] ?? -1]?.[2] ?? 0;
      });
      if (walked <= budgetM) return r;
      factor *= 2;
    }
   ...
   */
  streetNameAt(lon: number, lat: number, maxM = 40): string | null {
    const ei = this.nearestEdge(lon, lat, maxM);
    if (ei === null) return null;
    const e = this.g.edges[ei];
    if (!e) return null;
  ...
  loopRoute(
    ...
    const tried: { path: number[]; poi: PoiFeature | null; total: number; at: [number, number] }[] =
      [];
    let best: { path: number[]; poi: PoiFeature | null; total: number; at: [number, number] } | null =
      null;
    for (const c of candidates) {
      const [lon, lat] = c.at;
      let poiNode: number;
    ...
    const stress = (t: { path: number[] }): number => this.routeStress(t.path).avg;
    best =
      (near.length > 0
        ? [...near].sort((x, y) => stress(x) - stress(y))[0]
        : [...tried].sort(
            (x, y) => Math.abs(x.total - targetM) - Math.abs(y.total - targetM),
          )[0]) ?? null;
    ...
    const alternatives = (inBand.length > 0
      ...
      .filter((t) => {
        ...
        const bearing = (p: { at: [number, number] }): number =>
          Math.round(
            (Math.atan2(p.at[0] - start[0], p.at[1] - start[1]) * 180) / Math.PI / 45,
          );
        return bearing(t) !== bearing(best);
      });
    const build = (
      made: { path: number[]; poi: PoiFeature | null; total: number },
      id: RouteOption["id"],
...
export function routeCacheKey(opts: {
  ...
  walkMaxM: number;
  /** bumped whenever the rider changes what the router must avoid */
  avoidRevision: number;
}): string {
  const p = (c: [number, number]): string => `${c[0].toFixed(5)},${c[1].toFixed(5)}`;
  return [
    p(opts.from),
    p(opts.to),
    opts.profileId,
