...
export function snapToTrack(track, lon, lat, hintIdx = -1) {
    ...
    const search = (lo, hi) => {
        let best = { idx: Math.max(lo, 0), offM: Infinity, alongM: 0, pos: [lon, lat] };
...
export function trackBearingAhead(track, idx, alongM, aheadM = 60) {
    const { coords, cumM } = track;
    const a = coords[Math.max(0, Math.min(idx, coords.length - 2))];
    if (!a)
        return 0;
    let j = Math.min(idx + 1, coords.length - 1);
    while (j < coords.length - 1 && cumM[j] - alongM < aheadM)
        j++;
    const b = coords[j];
    if (!b)
        return 0;
...
export function trackSlice(track, uptoM) {
    const { coords, cumM } = track;
    const out = [];
