...
async function requestLoop() {
    ...
    try {
        ...
        if (!r)
            throw new Error("this area isn't mapped for routing yet");
        const { option, poi } = r.loopRoute([s.lng, s.lat], km * 1000, candidates, profileId, preferFlat);
        end?.remove();
        end = null;
        options = [option];
        loopParams = { km, kind };
...
let segPhotoTimer;
async function fetchSegmentPhoto(lon, lat) {
...
function fillSegmentPhoto(popup, lon, lat) {
    void fetchSegmentPhoto(lon, lat).then(({ url, captured }) => {
        if (popup !== hoverPopup)
            return; // hover moved on
        const slot = popup.getElement()?.querySelector("div[data-seg-photo]");
...
map.on("load", () => {
    ...
    for (const layer of ["construction-lines", "construction-pts"]) {
        map.on("click", layer, (e) => {
            ...
            new maplibregl.Popup()
                .setLngLat(e.lngLat)
                .setHTML(`🚧 <b>${props.name || props.kind || "construction"}</b><br>` +
    ...
    function constructionHtml(p) {
        ...
        const kind = typeof p["kind"] === "string" ? ` · ${p["kind"]}` : "";
        const address = typeof p["address"] === "string" && p["address"] !== "" ? `<br>${p["address"]}` : "";
        const detail = typeof p["detail"] === "string" && p["detail"] !== "" ? `<br>${p["detail"]}` : "";
...
for (const id of ["about-btn", "about-top"]) {
    el(id).addEventListener("click", openAbout);
}
...
function startRecording() {
    ...
    void (async () => {
        if (isNativeApp()) {
            recordBgWatcherId = await startBackgroundWatcher("Family Bike Router", "Recording your ride", recordOnFix, (message) => {
...
function stopRecording() {
    ...
    recordWatchId = null;
    if (recordBgWatcherId !== null)
        void stopBackgroundWatcher(recordBgWatcherId);
