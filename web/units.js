...
export function fmtDistTight(m) {
    if (current === "imperial") {
        const ft = m * FT_PER_M;
        if (ft < FT_SWITCH)
 ...
 */
export function navRound(m) {
    if (m < 15)
        return 0; // "now"
    if (current === "imperial") {
        const ft = m * FT_PER_M;
        if (ft < 300)
            return Math.round(ft / 50) * 50 / FT_PER_M;
        if (ft < 1000)
            return Math.round(ft / 100) * 100 / FT_PER_M;
        const mi = ft / 5280;
