...
function shapeOfTheProblem(s, afterClaim) {
    ...
    if (strandedShare >= 0.3) {
        return (`${budget} The streets they can use don't join up: ${mi(s.pocket_km)} of` +
            ` them sit in ${s.pockets} pockets with no safe way out.`);
    }
    return (`${budget} Most of the safe network here does join up — ${mi(s.connected_km)}` +
...
function wireLayerToggles(map) {
    ...
    for (const [box, layers] of pairs) {
        ...
        input.addEventListener("change", () => {
            ...
            if (box === "show-islands" || box === "show-barriers")
                syncCasings(map);
        });
    }
    const aerial = el("show-aerial");
...
function syncCasings(map) {
    const on = el("show-aerial").checked;
    for (const [box, casing] of [
        ["show-islands", "islands-casing"],
        ["show-barriers", "barriers-casing"],
    ]) {
        const shown = on && el(box).checked;
